...
app.delete('/mahasiswa', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Gagal menghapus semua mahasiswa', error });
  }
});
...
// DELETE
app.delete('/matakuliah', async (req, res) => {
  try {
    await Matakuliah.deleteMany({});
    res.json({ message: 'Semua data matakuliah berhasil dihapus' });
  } catch (error) {
    res.status(500).json({ message: 'Gagal menghapus semua matakuliah', error });
  }
});
